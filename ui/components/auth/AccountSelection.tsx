...
import { Plus, LockOpen, ChevronLeft } from "@gravity-ui/icons";
import { useState } from "react";
import { Account, loginWithAccount } from "@/hooks";
...
export function AccountSelection({
  ...
  const [loginStage, setLoginStage] = useState<"selection" | "password">("selection");
    ...
    <div className="relative z-10 min-h-screen flex items-center justify-center p-4 ">
      {loginStage === "selection" ? (
        <div className="w-full max-w-md space-y-4 animate-in slide-in-from-left-8 fade-in duration-300">
          {/* Header */}
          <div>
            <h1 className="text-2xl font-bold text-foreground">Select Account</h1>
          ...
          <Card className="w-full bg-surface border border-border shadow-surface">
            <Card.Content className="p-0">
              <ListBox
                ...
                selectedKeys={new Set(selectedAccountId ? [selectedAccountId] : [])}
                ...
                {accounts.map((account) => (
                  ...
                  >
                    <div className="flex items-center gap-3 w-full">
                      <Avatar size="sm" className="flex-shrink-0 bg-default text-default-foreground">
                        <Avatar.Fallback>
          ...
          >
            <div className="flex flex-col items-center justify-center gap-3">
              ...
              <div className="text-center">
                <p className="font-medium text-sm text-foreground">Add Account</p>
                <p className="text-xs text-muted">Sign in with a different account</p>
              </div>
            </div>
          </button>
        </div>
        ...
        <div className="w-full max-w-md space-y-4 animate-in slide-in-from-right-8 fade-in duration-300">
          ...
          <div>
            ...
            <p className="text-sm text-muted mt-1">
              Unlock <span className="text-foreground font-medium">{accounts.find(a => a.user_id.toString() === selectedAccountId)?.username}</span>
            </p>
          </div>
