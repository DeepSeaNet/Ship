...
function MentionText({ content, ownUsername, isOwn }: { content: string; ownUsername: string; isOwn: boolean }) {
...
export function MessageItem({ message, onReply, onEdit }: MessageItemProps) {
  ...
  const ownUsername = typeof window !== 'undefined' ? localStorage.getItem('username') || '' : '';

  const { actions, handleAction } = useMessageActions(message, { onReply, onEdit });
            ...
            >
              ...
              <div className="flex flex-wrap items-end justify-end gap-x-2 gap-y-1">
                <p className="text-sm break-all whitespace-pre-wrap flex-1 min-w-[10px]">
                  <MentionText content={message.content} ownUsername={ownUsername} isOwn={isOwn}/>
                </p>
                <div className={`flex items-center gap-1 shrink-0 mb-[-2px] ${isOwn ? 'ml-auto' : ''}`}>
                  ...
                  {isOwn && message.status && (
                    <div className="flex items-center opacity-60">
                      ...
                      {message.status === 'read' && <CheckDouble className="w-3.5 h-3.5" />}
                    </div>
                  )}
                </div>
              </div>
