...
export function useNotificationSettings() {
  ...
  const updateChatSetting = useCallback(<K extends keyof ChatNotificationSettings>(
    chatId: string,
    key: K,
    value: ChatNotificationSettings[K]
  ) => {
    setSettingsState(prev => {
      const current = prev.chatOverrides[chatId] || { muted: false, mentionsOnly: false };
      const next = {
        ...prev,
        chatOverrides: {
          ...prev.chatOverrides,
  ...
  return { settings, updateSetting, updateChatSetting, resetChatSettings, resetSettings };
}
