...
import { AES_KEY_SIZE, NONCE_SIZE, MAX_SKIP, DEFAULT_MAX_EPOCHS } from './constants';
import { deriveRatchetKeys, deriveMessageKey, aesGcmDecrypt } from './hkdf';
...
export class ReceiverCryptoRatchet {
    ...
    static async fromSecret(
        ...
        const ratchet = new ReceiverCryptoRatchet(senderPublicKey, senderId, maxPreviousEpochs);
    ...
    private async deriveMessageKeyForGeneration(
        ...
        if (targetGeneration < keys.generation) {
            ...
            throw new Error(`Message key for generation ${targetGeneration} not available (current: ${keys.generation})`);
        }

        // Skip forward if needed
        if (targetGeneration > keys.generation) {
            if (targetGeneration - keys.generation > MAX_SKIP) {
                throw new Error(`Too many skipped messages: ${targetGeneration - keys.generation}`);
            }

            while (keys.generation < targetGeneration) {
                const { newChainKey, messageKey } = await deriveMessageKey(keys.rootKey, keys.chainKey);
    ...
    async decrypt(data: Uint8Array): Promise<Uint8Array> {
        ...
        if (receivedUserId !== this.senderId) {
            throw new Error(`Incorrect sender ID. Expected ${this.senderId}, got ${receivedUserId}`);
        }
        ...
        const messageKey = await this.deriveMessageKeyForGeneration(epoch, generation);
