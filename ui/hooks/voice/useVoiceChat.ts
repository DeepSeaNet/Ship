...
export function useVoiceChat(): UseVoiceChatReturn {
	...
	const startCall = useCallback(
			...
			try {
				...
				const signaling = new GrpcSignalingAdapter({
					...
					onProducerRemoved: (producerId, participantId) => {
						...
						mediasoupServiceRef.current
							?.getConsumers()
							.forEach((consumer: any) => {
								if (consumer.producerId === producerId) {
									mediasoupServiceRef.current?.removeConsumer(
										consumer.id,
										handleTrackRemoved,
									);
								}
					...
					onMessage: async (msg) => {
						...
						if (
							mediasoupServiceRef.current?.handleCallback(
								msg.action,
								msg as any,
							)
						) {
						...
						if (msg.action === "Init") {
							...
							try {
								...
								await mediasoupServiceRef.current?.initializeDevice(
		...
		[status, addLog, handleTrackAdded, handleTrackRemoved, userId],
	); // dependencies
