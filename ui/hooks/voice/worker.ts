...
function isVp8Frame(chunk: RTCEncodedVideoFrame, metadata: RTCEncodedVideoFrameMetadata): boolean {
...
self.onmessage = async (event: MessageEvent) => {
    ...
    switch (type) {
        ...
        case "updateSsrcMapping": {
            ...
            console.log("[Worker] SSRC mapping updated:", ssrcToUserId.size, "entries");
            break;
        }
...
if ((self as any).RTCTransformEvent) {
    (self as any).onrtctransform = (event: any) => {
        ...
        console.log(
            ...
            mode === "decrypt" ? `senderId=${options.senderId ?? "from SSRC map"}` : "",
        );
        ...
        readable
            ...
            .pipeTo(writable)
            .catch((e: Error) => console.error(`[Worker] ${mode} pipeline error:`, e));
    };
} else {
    console.error("[Worker] RTCRtpScriptTransform not supported in this environment.");
}